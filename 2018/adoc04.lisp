...
(defstruct entry
  type             ; Entry type :guard, :wake-up, :fall-asleep
  original-string  ; Original entry string
  time-string      ; Original time string
  time-universal   ; Time string converted to universal time
  guard-id)        ; If type of :guard then stores guard id
...
(defun sort-entries (entries)
  (stable-sort entries #'< :key #'entry-time-universal))

(defun get-entry-time-minute (entry)
  (nth 1 (multiple-value-list (decode-universal-time (entry-time-universal entry)))))

(defun update-guard-sleep-tables
  (guard-id
  ...
  (let ((sleep-start-minute (get-entry-time-minute start-sleep-entry))
        ...
        (nap-minutes (if (gethash guard-id guards-asleep-minutes-table)
                       ...
                       (setf (gethash guard-id guards-asleep-minutes-table) (make-hash-table)))))

    (if (not (gethash guard-id guards-asleep-time-table))
      (setf (gethash guard-id guards-asleep-time-table) 0))
    (setf (gethash guard-id guards-asleep-time-table)
          (+ (gethash guard-id guards-asleep-time-table)
             (- sleep-end-minute sleep-start-minute)))

    (decf sleep-end-minute)
    (loop for m from sleep-start-minute to sleep-end-minute do
          (if (gethash m nap-minutes)
...
(defun solve1 (entries)
  (let ((current-guard-entry nil)
        ...
        (guards-asleep-minutes-table (make-hash-table)))

    (dolist (entry entries)
      (cond

        ; waiting for first guard entry
        ((not current-guard-entry)
         (if (eq (entry-type entry) :guard)
           (setf current-guard-entry entry)
           (progn
             (format t "ERROR: First entry should be Guard type, but got: ~a~%" entry)
             (break))))

        ; when hit guard entry having already some guard entry
        ((eq (entry-type entry) :guard)
         ; if current guard is sleeping recalculate its sleep time
         (if current-sleep-entry
           (update-guard-sleep-tables
             (entry-guard-id current-guard-entry)
             guards-asleep-time-table
             guards-asleep-minutes-table
             current-sleep-entry
             entry))
        ...
        ((eq (entry-type entry) :wake-up)
         (update-guard-sleep-tables
           ...
           current-sleep-entry
           entry)
         (setf current-sleep-entry nil))

        ; if guard falls asleep
