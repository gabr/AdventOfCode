...
fn isEquationPossible(test_value: u64, nums: []u64) bool {
    ...
    while (true) {
        const val = executeEquation(ops, nums);
...
fn solve(reader: anytype) !u64 {
    ...
    while(line_it.next()) |line| {
        ...
        if (isEquationPossible(test_value, num_al.items)) {
            sum += test_value;
        }
    }
    return sum;
...
test "07a example.a.txt" { try test_solve(3749,          "./07/example.a.txt"); }
test "07a input.txt"     { try test_solve(1985268524462, "./07/input.txt"); }
