...
const Machine = struct {
    ...
    pub fn fromLines(
        ...
        res.prize[0]+=10000000000000;
        res.prize[1]+=10000000000000;
        return res;
    }

    fn extractXY(line: []const u8, header: []const u8) ![2]i128 {
        const trimmed = line[header.len..];
        var it = std.mem.splitSequence(u8, trimmed, ", ");
        const x_str = it.next().?;
        const y_str = it.next().?;
        const x = try std.fmt.parseInt(i128, x_str["X=".len..], 10);
        const y = try std.fmt.parseInt(i128, y_str["Y=".len..], 10);
        return .{ x, y };
    }
    ...
    pub fn solve(self: Machine) ?[2]u128 {
        var A: [2][3]f128 = .{
        ...
        // validation
        dprint("x: [{d:.3}, {d:.3}]\n", .{x[0], x[1]});
        const a: i128 = @intFromFloat(@round(x[0]));
        const b: i128 = @intFromFloat(@round(x[1]));
        if (a < 0 or b < 0) {
...
test "example1" { try test_solve(875318608908,   "./13/example1.txt"); }
test "input"    { try test_solve(99423413811305, "./13/input.txt"); }
