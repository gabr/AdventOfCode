...
namespace adoc08
{
  ...
  {
    public string CheckedRegisterName { get; private set; }
    public string ConditionText { get; private set; }
    public float Constant { get; private set; }

    public Condition(string conditionInTextFormat)
    {
      string[] split = conditionInTextFormat.Split(" ");

      CheckedRegisterName = split[0].Trim();
      ConditionText = split[1].Trim();
      Constant = float.Parse(split[2].Trim());
    }

    public bool IsTrue(Dictionary<string, float> registers)
    {
      float registerValue = 0;
      if (registers.ContainsKey(CheckedRegisterName))
        registerValue = registers[CheckedRegisterName];

      switch (ConditionText)
      {
        case "==": return registerValue == Constant;
        case "!=": return registerValue != Constant;
        case "<":  return registerValue <  Constant;
        case ">":  return registerValue >  Constant;
        case "<=": return registerValue <= Constant;
        case ">=": return registerValue >= Constant;
        default:
          throw new NotImplementedException("ERROR: Unknown condition: " + ConditionText);
      }
    }
  }
  ...
  public class Instruction
  {
    ...
    public string TargetRegisterName { get; private set; }
    public float Increment { get; private set; }
    public Condition Condition { get; private set; }

    public Instruction(string instructionInTextFormat)
    {
      _originalString = instructionInTextFormat;

      // b inc 5 if a > 1
      string[] split = instructionInTextFormat.Split(" ");

      TargetRegisterName = split[0].Trim();

      Increment = float.Parse(split[2].Trim());
      if (split[1] == "dec")
        Increment *= -1;

      Condition = new Condition(instructionInTextFormat.Split(" if ")[1]);
    }

    public void Execute(Dictionary<string, float> registers)
    {
      //Console.WriteLine("Before execution: " + _originalString);
      //foreach (var key in registers.Keys)
      //  Console.WriteLine($"{key}: {registers[key]}");

      if (false == Condition.IsTrue(registers))
        return;

      if (false == registers.ContainsKey(TargetRegisterName))
        registers.Add(TargetRegisterName, 0);

      registers[TargetRegisterName] += Increment;

      //Console.WriteLine("After execution: " + _originalString);
      //foreach (var key in registers.Keys)
      //  Console.WriteLine($"{key}: {registers[key]}");
    }
  }
...
";
      ...
      //var instructions = ExtractInstructionsFromInput(test_instructions);
      var instructions = ExtractInstructionsFromInput(task_instructions);
      Console.WriteLine($"Extracted {instructions.Length} instructions");

      (var registers, float highestValueEver) = ProcessInstructions(instructions);
      foreach (var key in registers.Keys)
        Console.WriteLine($"{key}: {registers[key]}");

      (string registerName, float maxValue) = FindRegisterWithMaximumValue(registers);
      Console.WriteLine($"Maximum -> {registerName}: {maxValue}");
      Console.WriteLine($"Highest value ever -> {highestValueEver}");
    }

    static Instruction[] ExtractInstructionsFromInput(string input)
    {
      return input.Split("\n")
        .Select(l => l.Trim())
        .Where(l => l != "")
    ...
    static (Dictionary<string, float>, float highestValueEver) ProcessInstructions(Instruction[] instructions)
    {
      Dictionary<string, float> registers = new Dictionary<string, float>();
      float highestValueEver = 0; // not safe to assume 0

      void updateHighestValueEver()
      {
        foreach (var v in registers.Values)
          if (v > highestValueEver)
            highestValueEver = v;
      }
      ...
      {
        i.Execute(registers);
        updateHighestValueEver();
      }

      return (registers, highestValueEver);
    }

    static (string registerName, float registerValue) FindRegisterWithMaximumValue(Dictionary<string, float> registers)
    {
      float maxValue = registers.Values.Max();
      foreach (string registerName in registers.Keys)
        if (registers[registerName] == maxValue)
          return (registerName, maxValue);
