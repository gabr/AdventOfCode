...
namespace Solutions
{
    public class Day21
    {
        public void Split(ReadOnlySpan<char> input, string separator, out ReadOnlySpan<char> left, out ReadOnlySpan<char> right)
        {
            int index = input.IndexOf(separator[0]);

            left  = input.Slice(0, index - 1);
            right = input.Slice(index + separator.Length, input.Length - (index + separator.Length + 1));
        }

        public string GetNext(ref ReadOnlySpan<char> chars, string separator)
        {
            if (chars.IsEmpty)
                return null;

            int index = chars.IndexOf(separator[0]);
            string result = null;
            ...
            {
                result = chars.Slice(0, index).ToString();
                chars = chars.Slice(index + separator.Length);
            }

            return result;
        }

        public (Dictionary<string, List<HashSet<string>>> AllergentsToIngredients, Dictionary<string, int>IngredientsCount) MapFoodsByAllergents(string[] foods)
        {
            var allergentsToIngredients = new Dictionary<string, List<HashSet<string>>>();
            var ingredientsCount        = new Dictionary<string, int>();

            string ingredient = null;
            string allergent  = null;

            ReadOnlySpan<char> ingredients;
            ReadOnlySpan<char> allergents;

            foreach (var food in foods)
            {
                Split(food.AsSpan(), "(contains ", out ingredients, out allergents);

                var foodIngredients = new HashSet<string>();
                while ((ingredient = GetNext(ref ingredients, " ")) != null)
                {
                    foodIngredients.Add(ingredient);

                    if (ingredientsCount.ContainsKey(ingredient) == false)
                        ingredientsCount.Add(ingredient, 0);
                    ingredientsCount[ingredient] += 1;
                }

                while ((allergent = GetNext(ref allergents, ", ")) != null)
                {
                    if (allergentsToIngredients.ContainsKey(allergent) == false)
                        allergentsToIngredients.Add(allergent, new List<HashSet<string>>());

                    allergentsToIngredients[allergent].Add(foodIngredients);
                }
            }

            return (
                allergentsToIngredients,
                ingredientsCount
            );
        }
        ...
        public (Dictionary<string, HashSet<string>> PotentialAllergentsIngredientsMap, HashSet<string> PotentialAllergentsIngredientsSet) ExtractPotentialAllergents(Dictionary<string, List<HashSet<string>>> allergentsToIngredients)
        {
            ...
            {
                var potentialAllergentIngredients = new HashSet<string>();

                var allergentIngredients = allergentsToIngredients[allergent];
                foreach (var ingredients in allergentIngredients)
                foreach (var ingredient in ingredients)
                    if (potentialAllergentIngredients.Contains(ingredient) == false &&
                        allergentIngredients.All(i => i.Contains(ingredient)))
                    {
                        potentialAllergentIngredients.Add(ingredient);
                        potentialAllergentsIngredientsSet.Add(ingredient);
                    }
