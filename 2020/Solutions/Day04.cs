...
namespace Solutions
{
    public class Day04
    {
        private ReadOnlySpan<char> MoveToNextPassport(ref ReadOnlySpan<char> passports)
        {
            ReadOnlySpan<char> emptyLine = stackalloc char[] { '\r', '\n', '\r', '\n' };

            int index = passports.IndexOf(emptyLine);

            var passport = passports.Slice(0, index);
            passports    = passports.Slice(index + emptyLine.Length);

            return passport;
        }

        private bool HasAllRequiredFields(ReadOnlySpan<char> passport, string[] requiredFields)
        {
            for (int i = 0; i < requiredFields.Length; i++)
                if (passport.IndexOf(requiredFields[i]) == -1)
                    return false;

            return true;
        }

        public long Solve1(ReadOnlySpan<char> passports)
        {
            ...
            {
                var passport = MoveToNextPassport(ref passports);
                if (HasAllRequiredFields(passport, requiredFields))
        ...
        public ref struct Passport
        {
            public ReadOnlySpan<char> byr;
            public ReadOnlySpan<char> iyr;
            public ReadOnlySpan<char> eyr;
            public ReadOnlySpan<char> hgt;
            public ReadOnlySpan<char> hcl;
            public ReadOnlySpan<char> ecl;
            public ReadOnlySpan<char> pid;
        }

        public ReadOnlySpan<char> ExtractFieldValue(ReadOnlySpan<char> fieldName, ReadOnlySpan<char> passportCharacters)
        {
            int index = 0;

            if ((index = passportCharacters.IndexOf(fieldName)) == -1)
                return ReadOnlySpan<char>.Empty;

            int start = index + fieldName.Length + 1;
            int end = start + 1;

            while (end < passportCharacters.Length &&
                   passportCharacters[end] != ' ' &&
                   passportCharacters[end] != '\r')
                end += 1;

            return passportCharacters.Slice(start, end - start);
        }

        public bool ParsePassport(ReadOnlySpan<char> passportCharacters, ref Passport passport)
        {
            ReadOnlySpan<char> fieldValue;

            fieldValue = ExtractFieldValue("byr", passportCharacters);
            if (fieldValue.IsEmpty) return false;
            passport.byr = fieldValue;

            fieldValue = ExtractFieldValue("iyr", passportCharacters);
            if (fieldValue.IsEmpty) return false;
            passport.iyr = fieldValue;

            fieldValue = ExtractFieldValue("eyr", passportCharacters);
            if (fieldValue.IsEmpty) return false;
            passport.eyr = fieldValue;

            fieldValue = ExtractFieldValue("hgt", passportCharacters);
            if (fieldValue.IsEmpty) return false;
            passport.hgt = fieldValue;

            fieldValue = ExtractFieldValue("hcl", passportCharacters);
            if (fieldValue.IsEmpty) return false;
            passport.hcl = fieldValue;

            fieldValue = ExtractFieldValue("ecl", passportCharacters);
            if (fieldValue.IsEmpty) return false;
            passport.ecl = fieldValue;

            fieldValue = ExtractFieldValue("pid", passportCharacters);
            if (fieldValue.IsEmpty) return false;
        ...
        public bool ValidateYear(in ReadOnlySpan<char> yearCharacters, int min, int max)
        {
            if (yearCharacters.Length != 4)
                return false;

            int year;
            if(int.TryParse(yearCharacters, out year) == false)
                return false;

            return year >= min && year <= max;
        }

        public bool ValidateByr(in Passport passport) => ValidateYear(in passport.byr, 1920, 2002);
        public bool ValidateIyr(in Passport passport) => ValidateYear(in passport.iyr, 2010, 2020);
        public bool ValidateEyr(in Passport passport) => ValidateYear(in passport.eyr, 2020, 2030);

        public bool ValidateHgt(in Passport passport)
        {
            if (passport.hgt.Length < 4)
                return false;

            int height;
            if (int.TryParse(passport.hgt.Slice(0, passport.hgt.Length - 2), out height) == false)
                return false;
        ...
        public bool ValidateHcl(in Passport passport)
        {
        ...
        public bool ValidateEcl(in Passport passport)
        {
            var validValues = new []
            {
                "amb",
            ...
            for (int i = 0; i < validValues.Length; i++)
                if (passport.ecl.IndexOf(validValues[i]) != -1)
                    return true;

            return false;
        }

        public bool ValidatePid(in Passport passport)
        {
            if (passport.pid.Length != 9)
                return false;

            for (int i = 0; i < passport.pid.Length; i++)
                if ((passport.pid[i] >= '0' && passport.pid[i] <= '9') == false)
                    return false;

            return true;
        }

        public long Solve2(ReadOnlySpan<char> passports)
        {
            ...
            while (passports.Length > 0)
            {
                var passportCharacters = MoveToNextPassport(ref passports);
                if (ParsePassport(passportCharacters, ref passport) &&
                    ValidateByr(in passport) &&
                    ValidateIyr(in passport) &&
                    ValidateEyr(in passport) &&
                    ValidateHgt(in passport) &&
                    ValidateHcl(in passport) &&
                    ValidateEcl(in passport) &&
                    ValidatePid(in passport))
                        validPassportsCount += 1;
            }
