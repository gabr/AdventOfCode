...
namespace Solutions
{
    public class Day07
    {
        ...
        {
            public List<BagRegulation> ParentsRegulations { get; set; } = new List<BagRegulation>();
            public List<BagRegulation> ChildRegulations { get; set; } = new List<BagRegulation>();

            public override string ToString()
            {
                return
                    $"Bag: '{Color}'" +
                    $"\n  Parents ({ParentsRegulations.Count}): {string.Join(", ", ParentsRegulations)}" +
                    $"\n  Childs  ({ChildRegulations.Count}): {string.Join(", ", ChildRegulations)}";
            }
        }

        public record BagRegulation(int Count, Bag Child, Bag Parent)
        {
            public override string ToString()
            {
                return $"BagRegulation: {Count}, '{Parent.Color}' -> '{Child.Color}'";
            }
        }


        public Dictionary<string, Bag> ParseBagsRegulations(string[] regulations)
        {
            ...
            foreach (var regulationString in regulations)
            {
                var split = regulationString.Split(" bags contain ");
                var bagColor = split[0];

                var parentBag = GetBag(bagColor);

                if (split[1] != "no other bags.")
                {
                                       // remove the dot at the end of the string
                    foreach (var r in (split[1].Substring(0, split[1].Length - 1)).Split(", "))
                    {
                        int firstSpaceIndex = r.IndexOf(" ");
                        int lastSpaceIndex  = r.LastIndexOf(" ");

                        var count = int.Parse(r.Substring(0, firstSpaceIndex));
                        var color = r.Substring(firstSpaceIndex + 1, lastSpaceIndex - (firstSpaceIndex + 1));

                        var childBag   = GetBag(color);
                        var regulation = new BagRegulation(count, childBag, parentBag);

                        parentBag.ChildRegulations.Add(regulation);
                        childBag.ParentsRegulations.Add(regulation);
                    }
                }
            }
        ...
        public int Solve1(string[] regulations, string bagColor)
        {
            var bagsByColor = ParseBagsRegulations(regulations);
            var rootBag     = bagsByColor[bagColor];

            var bagsUniqueColors = new HashSet<string>();
            void GetAllParentsColors(Bag bag)
            {
                //Console.WriteLine(bag);
                foreach (var regulation in bag.ParentsRegulations)
                {
                    //Console.WriteLine(regulation);
                    if (regulation.Child.Color == bag.Color)
                    {
                        bagsUniqueColors.Add(regulation.Parent.Color);
                        GetAllParentsColors(regulation.Parent);
                    }
                }
            }

            GetAllParentsColors(rootBag);
            return bagsUniqueColors.Count;
        }


        public int Solve2(string[] regulations, string bagColor)
        {
            var bagsByColor = ParseBagsRegulations(regulations);
            var rootBag     = bagsByColor[bagColor];

            var bagsCount = 0;
            void CountRequiredBags(Bag bag)
            {
                //Console.WriteLine(bag);
                foreach (var regulation in bag.ChildRegulations)
                {
                    //Console.WriteLine(regulation);
                    bagsCount += regulation.Count;
                    for (int i = 0; i < regulation.Count; i++)
                        CountRequiredBags(regulation.Child);
                }
            }

            CountRequiredBags(rootBag);
            return bagsCount;
        }
