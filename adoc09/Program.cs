...
/*
  ...
  Each group is assigned a score which is one more than the score of the group that
...
namespace adoc09
{
  public class Node
  {
    ...
    public int CalculateGroupsScore()
    {
      return NestLevel + _childs.Sum(c => c.CalculateGroupsScore());
    }

    public static (Node rootNode, int removedGarbageCount) ParseInputToNodes(string input)
    {
      //Console.WriteLine("Before cleanup: " + input);

      // clearing input
      int removedGarbageCount = 0;
      StringBuilder sb = new StringBuilder();
      bool insideGarbage = false;
      for (int i = 0; i < input.Length; i++)
      {
        if (false == insideGarbage && input[i] == '<')
        {
          insideGarbage = true;
          continue;
        }
      ...
      string clearInput = sb.ToString();
      //Console.WriteLine("After cleanup: " + clearInput);

      string[] SplitGroups(string toSplit)
      {
        ...
        for (int i = 0; i < toSplit.Length; i++)
        {
          ...
          else if (toSplit[i] == '}')
          {
            openCount -= 1;
            if (openCount == 0)
              splits.Add(toSplit.Substring(openIndex, i - openIndex + 1));
          }
        }
      ...
      Node GetChilds(string clearedInput, int nestLevel)
      {
        Node child = new Node() { NestLevel = nestLevel };

        //Console.WriteLine($"Before split ({nestLevel}): " + clearedInput);
        string[] split = SplitGroups(clearedInput.Substring(1, clearedInput.Length - 2));

        foreach (string s in split)
        {
          Node c = GetChilds(s, nestLevel + 1);
          child._childs.Add(c);
        }
      ...
      return (GetChilds(clearInput, 1), removedGarbageCount);
    }
  }
  ...
  class Program
  {
    ...
    static void Test()
    {
      ...
      foreach (var ti in test_inputs)
      {
        (Node root, int removedGarbageCount) = Node.ParseInputToNodes(ti.input);
        int score = root?.CalculateGroupsScore() ?? - 1;

        if (score != ti.score || removedGarbageCount != ti.removedGarbageCount)
        {
          Console.WriteLine("Test FAILED:\n" +
            $"  Input: '{ti.input}'\n" +
            $"  Expected score:   {ti.score}\n" +
            $"  Calculated score: {score}\n" +
            $"  Removed garbage count:\n" +
            $"    Expected: {ti.removedGarbageCount}\n" +
            $"    Got:      {removedGarbageCount}\n");
          failedTestsCount += 1;
        }
      }
      ...
      if (failedTestsCount == 0)
        ...
        Console.WriteLine($"{failedTestsCount} tests FAILED!");
    }
  }
}
